...
describe('parseSidInput', () => {
  it('aceita valor puro decodificado (0:hex)', () => {
    expect(parseSidInput(RAW_SID, 'br142')).toEqual({ sid: RAW_SID, extraCookies: [] });
  });

  it('aceita valor puro URL-encoded (0%3Ahex) e decodifica', () => {
    expect(parseSidInput(ENCODED_SID, 'br142')?.sid).toBe(RAW_SID);
  });

  it('aceita o export completo do EditThisCookie e extrai o sid do mundo', () => {
    const result = parseSidInput(EDIT_THIS_COOKIE_EXPORT, 'br142');
    expect(result?.sid).toBe(RAW_SID);
  ...
  it('aceita cookie único em JSON', () => {
    ...
    expect(parseSidInput(single, 'br142')?.sid).toBe(RAW_SID);
  });

  it('prefere o sid do domínio do mundo quando há vários', () => {
    const multi = JSON.stringify([
      { domain: 'br140.tribalwars.com.br', name: 'sid', value: '1:1111111111111111111111111111111111111111111111111111111111111111' },
      { domain: 'br142.tribalwars.com.br', name: 'sid', value: ENCODED_SID },
    ]);
    expect(parseSidInput(multi, 'br142')?.sid).toBe(RAW_SID);
  });
