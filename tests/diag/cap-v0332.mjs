// QA visual 0.33.1 (diagnóstico enquadramento do dono): captura sg2/guerra em
// 1900px fullPage com o snapshot de tropas REAL do dono (cópia de leitura do
// JSON dele) num userData temporário + API auth LOCAL (nada toca a sessão dele).
...
const PAGES = [
  { id: 'sg2', h: 1100, delay: 2500 },
  { id: 'guerra', h: 1100, delay: 9000 },
  { id: 'dashboard', h: 1100, delay: 1500 },
];
const THEMES = ['claro'];

const dir = mkdtempSync(join(tmpdir(), 'shs-v0332-'));
...
// Snapshot REAL do dono (dados que ele coletou — o Resumo Geral dele populado).
const snapshotOrigem = join(process.env.APPDATA ?? '', 'Staff Hub Toxic Squad', 'stores', 'troops-snapshots.json');
if (existsSync(snapshotOrigem)) {
  copyFileSync(snapshotOrigem, join(userData, 'stores', 'troops-snapshots.json'));
  console.log('snapshot de tropas do dono copiado');
} else {
  console.log('AVISO: sem troops-snapshots.json no userData do dono');
}
// Rascunho de guerra com escala real (2428×183) para a página guerra.
...
const outDir = join(root, 'tests/diag/cap-v0332');
...
try {
  ...
  console.log(`QA VISUAL 0.33.2 diag: ${total} capturas em ${outDir}`);
} finally {
  server.kill();
  await dormir(300);
