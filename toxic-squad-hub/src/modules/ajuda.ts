...
export function renderAjuda(container: HTMLElement): void {
  ...
  item(agendador.body, '**O envio sai da Praça da aldeia de origem.** Basta deixar uma aba do jogo aberta (em qualquer tela, pode ficar em segundo plano): **60 s antes** ela vai sozinha até a Praça certa, envia no ms e depois **volta** para onde você estava. Uma faixa avisa 15 s antes, com "Ir agora" e "Não levar esta aba". Aba em uso (texto sendo escrito, comando sendo montado) nunca é levada.');
  ...
  item(agendador.body, '**OP com várias origens:** a ida automática atende **uma origem por vez**, com pelo menos 30 s entre os envios. Origens coladas precisam de uma aba aberta na Praça de cada uma — a seção Comandos marca "Pronta na Praça" ou "Sem aba na Praça" em cada comando.');
