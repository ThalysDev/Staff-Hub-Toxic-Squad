...
export function renderAjuda(container: HTMLElement): void {
  ...
  item(agendador.body, '**Central de comandos em abas:** **Fila** (busca, filtro por tipo, pausar/cancelar em lote, exportar/importar), **Novo comando**, **Em bloco**, **Mapa** e **Histórico** (precisão de cada envio: "+3 ms do horário marcado", e o motivo das falhas). Na Fila, **Mudar horário** e **Duplicar**; no Histórico, **Usar de novo**.');
  ...
  item(agendador.body, '**Na Fila:** "Mudar horário" ajusta sem recriar (pela chegada, se o comando foi criado assim); "Duplicar"/"Usar de novo" copiam o comando para o formulário (confira o horário); "Cancelar envio" deixa o registro no histórico.');
  item(agendador.body, '**Bônus noturno:** o resumo avisa quando um ataque chega na janela do bônus noturno do mundo (defesa em dobro).');
